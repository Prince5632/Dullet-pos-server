...
const createUser = async (req, res) => {
  try {
    ...
    if (otherDocumentsMeta) {
      try {
        parsedOtherDocumentsMeta =
          typeof otherDocumentsMeta === "string"
    ...
    const buildDocumentPayload = (file, defaultType) =>
      file
        ? {
            buffer: file.buffer,
    ...
    const aadhaarDoc = buildDocumentPayload(
...
const updateUser = async (req, res) => {
  try {
    ...
    const parsedDocumentRemovals = parseJsonField(
      "removeDocumentIds",
      "Invalid document removal payload"
    );
    if (parsedDocumentRemovals !== undefined) {
      updateData.removeDocumentIds = parsedDocumentRemovals;
    }

    const parsedOtherDocumentsMeta = parseJsonField(
      "otherDocumentsMeta",
      "Invalid other documents metadata"
    );

    const buildDocumentPayload = (file, defaultType, index = 0) =>
      file
        ? {
            buffer: file.buffer,
            mimetype: file.mimetype,
            ...
            label:
              parsedOtherDocumentsMeta?.[index]?.label || file.originalname,
          }
        : null;

    if (req.files?.aadhaarDocument?.[0]) {
      updateData.aadhaarDocument = buildDocumentPayload(
    ...
    if (req.files?.panDocument?.[0]) {
      updateData.panDocument = buildDocumentPayload(
    ...
    if (req.files?.otherDocuments?.length) {
      updateData.otherDocuments = req.files.otherDocuments.map((file, index) =>
        buildDocumentPayload(file, "other", index)
      );
    }
