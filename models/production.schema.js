  ...
  if (!this.batchId) {
    try {
      ...
      while (!isUnique && attempts < maxAttempts) {
        ...
        const existingBatch = await this.constructor
          .findOne({ batchId })
          .lean();
        if (!existingBatch) {
          isUnique = true;
...
productionSchema.virtual("totalOutputQty").get(function () {
  return this.outputDetails.reduce(
    (total, output) => total + output.productQty,
    0
  );
});
...
productionSchema.virtual("conversionEfficiency").get(function () {
  ...
  return this.inputQty > 0
    ? ((totalOutput / this.inputQty) * 100).toFixed(2)
    : 0;
});
